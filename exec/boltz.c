...
int main(int argc, char **argv) {
  time_t main_code_timer, function_time;
  main_code_timer = clock();
  initialize_func_timer();
  ...
  if (weightgenFlag == 0) {
    ...
    if(isoFlag == 1) { //load AnIso weights generated by the AnIso weight generator (seperate program)
      ...
      printf("Initializing weights for Iso case...\n");
      for(i=0;i<num_species;i++)
        for(j=0;j<num_species;j++) {
          initialize_weights(N, zeta, L_v, lambda, weightFlag, isoFlag, mixture[i], mixture[j], weightgenFlag, conv_weights[j*num_species + i]);
        }
    }
  }
  else {
    for (i = 0; i < num_species; i++)
    for (j = 0; j < num_species; j++) {
      initialize_weights(N, zeta, L_v, lambda, weightFlag, isoFlag, mixture[i], mixture[j], weightgenFlag);
    }
  }
