...
void allocate_hom(int N, double **v, double **zeta, double ***f, double ***f1,
                  double ***Q, int num_species) {
  int i;
...
void initialize_hom(int N, double L_v, double *v, double *zeta, double **f,
                    int initFlag, species *mixture) {
  int i, j, k;
  double BKWt, Temp, K;
  ...
  for (spec = 0; spec < numspec; spec++) {
      ...
      for (j = 0; j < N; j++)
        for (k = 0; k < N; k++)
          for (i = 0; i < N; i++)
            f[spec][k + N * (j + N * i)] =
                exp(-1 * S *
                    (sqrt(v[i] * v[i] + v[j] * v[j] + v[k] * v[k]) - sigma) *
                    (sqrt(v[i] * v[i] + v[j] * v[j] + v[k] * v[k]) - sigma) /
                    (sigma * sigma)) /
                (S * S);
        ...
        for (j = 0; j < N; j++)
          for (k = 0; k < N; k++) {
            f[spec][k + N * (j + N * i)] =
                (exp(-(v[i] * v[i] + v[j] * v[j] + v[k] * v[k]) /
                     (2 * K * Temp * Temp))) /
                (2.0 * pow(2 * M_PI * K * Temp * Temp, 1.5)) *
                ((5 * K - 3) / K +
                 (1 - K) * (v[i] * v[i] + v[j] * v[j] + v[k] * v[k]) /
                     (K * K * Temp * Temp));
          }
    ...
    case 3:
      ...
      for (j = 0; j < N; j++)
        for (k = 0; k < N; k++)
          for (i = 0; i < N; i++)
            f[spec][k + N * (j + N * i)] =
                pre * (exp(-((v[i] - 2.0 * sigma) * (v[i] - 2.0 * sigma) +
                             v[j] * v[j] + v[k] * v[k]) /
                           (2.0 * sigma * sigma)) +
                       exp(-((v[i] + 2 * sigma) * (v[i] + 2 * sigma) +
                             v[j] * v[j] + v[k] * v[k]) /
                           (2.0 * sigma * sigma)));
    ...
    case 4:
      ...
      printf("SimpleMax, rho=%g, T=%g, mass=%g KB=%g \n", rho, maxTemp,
             mixture[spec].mass, KB);
      for (j = 0; j < N; j++)
        for (k = 0; k < N; k++)
          for (i = 0; i < N; i++)
            f[spec][k + N * (j + N * i)] =
                (rho / mixture[spec].mass) *
                pow(0.5 * mixture[spec].mass / (M_PI * KB * maxTemp), 1.5) *
                exp(-(0.5 * mixture[spec].mass / (KB * maxTemp)) *
                    ((v[i]) * (v[i]) + v[j] * v[j] + v[k] * v[k]));
      // f[spec][k + N*(j + N*i)] = pow(1.0/M_PI,1.5)*exp(-(v[i]*v[i] +
      // v[j]*v[j] + v[k]*v[k]));
      break;
    ...
    case 5:
      ...
      for (j = 0; j < N; j++)
        for (k = 0; k < N; k++)
          for (i = 0; i < N; i++) {
            f[spec][k + N * (j + N * i)] =
                (1 + 0.1 * sin(v[i] * v[i] + v[j] * v[j] + v[k] * v[k])) *
                exp(-((v[i]) * (v[i]) + v[j] * v[j] + v[k] * v[k])) /
                (M_PI * sqrt(M_PI));
          }
...
void allocate_inhom(int N, int nX, double **v, double **zeta, double ****f,
                    double ****f_conv, double ****f_1, double ***Q, int Ns) {
  int i, j;
...
void initialize_inhom(int N, int Ns, double L_v, double *v, double *zeta,
                      double ***f, double ***f_conv, double ***f_1,
                      species *mixture, int initFlag, int nX, double *xnodes,
                      double *dxnodes, double dt, int *t, int order,
                      int restart, char *inputfilename) {
  int rank;
  ...
  initialize_transport(N, nX, L_v, xnodes, dxnodes, v, initFlag, dt, 1.0,
                       mixture);
  ...
  if (restart == 1) {
    ...
    *t = 0;
    ...
    case 2:
      ...
      ux_r = -1.0;
      printf(
          "Two Maxwellian problem?: rho:%g, ux_l:%g ux_r:%g T_l:%g, T_r:%g\n",
        ...
        case 1:
          for (i = 0; i < N; i++)
            for (j = 0; j < N; j++)
              for (k = 0; k < N; k++) {
                f[m][l][k + N * (j + N * i)] =
                    (rho_r / mixture[m].mass) *
                    pow(0.5 * mixture[m].mass / (M_PI * KB * maxTemp), 1.5) *
                    exp(-(0.5 * mixture[m].mass / (KB * maxTemp)) *
                        ((v[i]) * (v[i]) + v[j] * v[j] + v[k] * v[k]));
              }
          break;
        case 2:
          ...
          for (i = 0; i < N; i++)
            for (j = 0; j < N; j++)
              for (k = 0; k < N; k++) {
                f[m][l][k + N * (j + N * i)] =
                    rho_r *
                    exp(-(v[i] * v[i] + v[j] * v[j] + v[k] * v[k]) / T_r) /
                    ((T_r * M_PI) * sqrt(T_r * M_PI));
              }
          break;
        case 5: // Poiseuille
          for (i = 0; i < N; i++)
            for (j = 0; j < N; j++)
              for (k = 0; k < N; k++) {
                f[m][l][k + N * (j + N * i)] =
                    rho_l *
                    exp(-(v[i] * v[i] + v[j] * v[j] + v[k] * v[k]) / T_l) /
                    ((T_l * M_PI) * sqrt(T_l * M_PI));
              }
          break;
...
void dealloc_inhom(int nX, int order, double *v, double *zeta, double ***f,
                   double ***f_conv, double ***f_1, double **Q) {
  int i, j, l;
