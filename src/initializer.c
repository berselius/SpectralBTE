...
void allocate_hom(int N, double **v, double **zeta, double ***f, double ***f1, double ***Q, int num_species) {
    ...
    *Q = malloc(num_species*num_species * sizeof(double *));
    for(i = 0; i < num_species * num_species; i++) {
        (*Q)[i] = malloc(N3 * sizeof(double));
    }

    *f = malloc(num_species * sizeof(double *));
    for(i = 0; i < num_species; i++) {
        (*f)[i] = malloc(N3 * sizeof(double));
    }

    *f1 = malloc(num_species * sizeof(double *));
    for(i = 0; i < num_species; i++) {
        (*f1)[i] = malloc(N3 * sizeof(double));
    }
}
...
void initialize_inhom(int N, int Ns, double L_v, double *v, double *zeta, double ***f, double ***f_conv, double ***f_1, species *mixture, int initFlag, double lambda, int nX, double *xnodes, double *dxnodes, double dt, int *t, int order, int restart, int restart_time, char *inputfilename) {
    ...
    for(i=0;i<Ns;i++) {
        for(l = 0; l < (nX + 2 * order); l++) {
            f[i][l] = malloc(N3 * sizeof(double));
            f_conv[i][l] = malloc(N3 * sizeof(double));
            f_1[i][l] = malloc(N3 * sizeof(double));
        }
    }
