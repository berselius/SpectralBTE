...
void read_input(int *N, double *L_v, double *Kn, double *lambda, double *dt,
                ...
                int *homogFlag, int *weightFlag, int *isoFlag, char **meshFile,
                int *num_species, char ***species_names, char *inputFilename) {
  int i;
  char line[80] = {"dummy"};
  ...
  set_default_values(N, L_v, Kn, lambda, dt, nT, order, dataFreq, restart,
                     restart_time, bcFlag, homogFlag, weightFlag, num_species,
                     isoFlag, meshFile);
 ...
 */
void set_default_values(int *N, double *L_v, double *Kn, double *lambda,
                        double *dt, int *nT, int *order, int *dataFreq,
                        int *restart, double *restart_time, int *bcFlag,
                        int *homogFlag, int *weightFlag, int *num_species,
                        int *isoFlag, char **meshFile) {
  /*Assumes space-homogeneous problem*/
  *homogFlag = 0;

  /*Tells weights routine to check if weights exist with current parameters. If
   * not, regenerate them*/
  *weightFlag = 0;
