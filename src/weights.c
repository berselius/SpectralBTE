...
void initialize_weights_mpi(double nodes,double Lv, double lam,species species_i, species species_j,int weightFlag, int size){
    ...
    if (weightFlag == 0) {
        ...
        if (file == NULL) {
			double *conv_weights;
            write_weights_mpi(&conv_weights, buffer_weights, N, 0, N*N*N, size, 0);
        }
    }
    else {//weights forced to be regenerated
		double *conv_weights;
...
/*
 * loads stored weights into conv_weights array. Works with both Homogeneous and Inhomogenous cases
 * args:
