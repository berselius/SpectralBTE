...
// Initializes this module's static variables and allocates what needs
// allocating
...
static void find_maxwellians(double *M_mat, double *g_mat, double *mat) {
  ...
  for (index = 0; index < N * N * N; index++) {
    ...
    k = index - N * (j + N * i);
    M_mat[index] =
        prefactor * exp(-(0.5 / T) * ((v[i] - vel[0]) * (v[i] - vel[0]) +
                                      (v[j] - vel[1]) * (v[j] - vel[1]) +
                                      (v[k] - vel[2]) * (v[k] - vel[2])));
...
#pragma omp parallel for private(zeta_x, zeta_y, zeta_z, xi_x, xi_y, xi_z, x,  \
                                 y, z, conv_weight_chunk)
...
function fft3D
--------------
Computes the fourier transform of in, and adjusts the coefficients based on our
v, eta grids
*/
