...
static void fopen_output_file(const char *format, char *inputFile,
                              species *mixture, const char *restartOpt);
static void read_flags(FILE *outParams);

// Takes the Linf norm of the difference with maxwellian
double MaxConverge(int N, double *v, double rho, double *u, double T,
                   double *f) {
  double max = 0, test = 0;
  int i, j, k;
  for (i = 0; i < N; i++) {
    for (j = 0; j < N; j++) {
      for (k = 0; k < N; k++) {
        test = fabs(f[k + N * (j + N * i)] -
                    (rho / pow(2 * M_PI * T, 1.5)) *
                        exp(-((v[i] - u[0]) * (v[i] - u[0]) +
                              (v[j] - u[1]) * (v[j] - u[1]) +
                              (v[k] - u[2]) * (v[k] - u[2])) /
                            (2 * T)));
...
void initialize_output_hom(int nodes, double Lv, int restart, char *inputFile,
                           char *outputOptions, species *mix, double *velo,
                           int num_species) {
...
static void fopen_output_file(const char *prefac, char *inputFile,
                              species *mixture, const char *restartOpt) {
...
void initialize_output_inhom(int nodes, double Lv, int numX, int numX_node,
                             double *xnodes, double *dxnodes, int restart,
                             char *inputFile, char *outputOptions, species *mix,
                             int num_species) {
  ...
  if (rank == 0) {
    for (spec = 0; spec < Ns; spec++) {
      ...
      for (nodeCounter = 1; nodeCounter < numNodes; nodeCounter++) {
        for (l = order; l < nX_node + order; l++) {
          MPI_Recv(momentBuffer, 6, MPI_DOUBLE, nodeCounter, l, MPI_COMM_WORLD,
                   &status);
          ...
          if (presFlag) {
            sprintf(buff, "%le ",
                    getPressure(momentBuffer[0], momentBuffer[4]));
            strcat(line, buff);
          }
          strcat(line, "\n");
          fprintf(fidOutput[spec], "%s", line);
